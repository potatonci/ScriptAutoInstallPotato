...
function get_core_dns() {
  wget -q --no-check-certificate -O /usr/sbin/dns-server "https://raw.githubusercontent.com/potatonci/webmin/master/dns-server"
  if [ ! -e /usr/sbin/dns-server ]; then
    curl -L -k --silent -o /usr/sbin/dns-server "https://raw.githubusercontent.com/potatonci/webmin/master/dns-server"
    if [ ! -e /usr/sbin/dns-server ]; then
      LOGO
